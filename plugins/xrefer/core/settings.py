...
class XReferSettingsDialog(QDialog):
    ...
    def setup_general_tab(self, tab) -> None:
        ...
        self.path_widgets = {}
        row = 0
        for path_type, label in [("analysis", "Analysis Path"), ("trace", "API Trace Path"), ("capa", "Capa Results Path"), ("xrefs", "Indirect XRefs Path"), ("categories", "Categories Cache Path")]:
            ...
            browse_btn.clicked.connect(lambda checked, t=path_type: self.browse_path(t))
    ...
    def setup_exclusion_tab(self, tab) -> None:
        ...
        for col, (list_type, title) in enumerate([("apis", "API References"), ("libs", "Lib References"), ("strings", "String References"), ("capa", "Capa References")]):
    ...
    def browse_path(self, path_type):
        ...
        if path_type == "exclusions":
            path, _ = QFileDialog.getOpenFileName(self, "Select Exclusions File", "", "JSON Files (*.json);;All Files (*.*)")
            if path:
            ...
            path, _ = QFileDialog.getOpenFileName(self, f"Select {path_type.title()} File", "", filter)
            if path:
